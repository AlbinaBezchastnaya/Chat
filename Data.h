...
class User // класс с персональными данными пользователя
{
protected:
	string Login; 
	string Password; 
...
public:
	User();
	void initUser(int messageNum); // функция для регистрации пользователя
	string getCharValue(int fieldNumber) const;   // геттер для вывода полей класса типа string
	int getCharValue() const;                     // геттер для вывода поля класса типа int
	void setValue(int numValue, string value);    // сеттер для инициализации полей типа string
	void setValue(int value);                     // сеттер для инициализации полей типа int

};

class Message final : public User
{
public:
	void initMessage(string Login, int messageNum);
};
